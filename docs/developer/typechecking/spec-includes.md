...
sorted by `base` ascending, with no gaps. A [`Span`](https://mercorg.github.io/merc/merc_utilities/span/struct.Span.html)'s `start`/`end` are offsets
into this one space, not into any single file's own text — rendering a span
takes the `SourceMap` and looks up which file an offset falls into, by binary
search over `base`, before rendering the surrounding line. An offset past the
...
a `&mut SourceMap` rather than creating its own: pass the same `SourceMap` a
...
convenience wrapper over a throwaway `SourceMap`, fine for a caller that never
...
[`UntypedDataSpecification`](https://mercorg.github.io/merc/merc_syntax/syntax_tree/struct.UntypedDataSpecification.html) (and its [`UntypedProcessSpecification`](https://mercorg.github.io/merc/merc_syntax/syntax_tree/struct.UntypedProcessSpecification.html)/
[`UntypedStateFrmSpec`](https://mercorg.github.io/merc/merc_syntax/syntax_tree/struct.UntypedStateFrmSpec.html) counterparts) then drives a [`Resolver`](https://mercorg.github.io/merc/merc_syntax/imports/struct.Resolver.html) that walks the
...
For each file, the `Resolver` registers its text into `sources` — fixing its
...
than something [`Traverse`](https://mercorg.github.io/merc/merc_syntax/traverse/trait.Traverse.html) can do generically: `Traverse`'s
recursion only ever descends into children of the *same* node type (a
[`SortExpression`](https://mercorg.github.io/merc/merc_syntax/syntax_tree/type.SortExpression.html)'s children are other `SortExpression`s), so it can't reach a
...
behind `Unresolved` layers. This matters for a caller with access
to the `SourceMap` (an LSP): an `ImportError` carries the failing directive's
own quoted-path span, and can recover the underlying parser error —
line/column, expected tokens — through any number of nested `Unresolved`
...
- **A bare parse** — no `SourceMap` involved, spans meaningless outside the
  ...
  overhead.
- **A registered parse** — the same bundled text goes into `sources` under a
  ...
  (part of [`DataSpecification`](https://mercorg.github.io/merc/merc_typecheck/data_specification/struct.DataSpecification.html)'s own `system` field, built eagerly), and so is
...
for `List(Nat)`, another registration for `List(D)`, …) feeding the generated
content lowering produces. Confusing the two would be a category error — the
bare copy's spans are not meaningful against any `SourceMap` at all.
...
of `basics`. A `HashMap<(String, ResolvedSortId), Span>` on the checking
context is populated as a side effect of resolving `system`'s own signature
...
[Building `TypingInfo`](typing-info.md#lsp-support) consults this table as a
